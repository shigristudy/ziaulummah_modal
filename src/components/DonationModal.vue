      ...
      <div class="border-t border-gray-200 py-6 px-4 sm:px-10" v-if="projects">
        <div class="mb-4 ">
          <label for="select-category"
            class="required text-green dark:text-black font-bold block mb-2 text-lg">Category</label>
          <div class="grid">
            <select id="select-category" v-model="current_donation.category_id" @change="fetchProjects()"
              class="appearance-none row-start-1 col-start-1 form-select block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-green focus:outline-none"
              aria-label="Categories">
              <option :value="0">Select a Category</option>
              <option v-for="(cat, index) in categories" :key="'category_' + index" :value="cat.id">{{ cat.name }}
              </option>
            </select>
            <svg
              class="pointer-events-none z-10 right-1 relative col-start-1 row-start-1 h-4 w-4 self-center justify-self-end forced-colors:hidden"
        ...
        <div class="mb-4">
          <label for="select-project"
            class="required text-green dark:text-black font-bold block mb-2 text-lg">Project</label>
          <div class="grid">
            <select @change="projectChanged" id="select-project" v-model="current_donation.project_id"
              class="appearance-none row-start-1 col-start-1 form-select block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white  focus:outline-none"
              aria-label="Projects" :class="{ '!border-red': errors && errors.project_id }">
              <option :value="0">Select a project</option>
              <option v-for="(project, index) in projects" :key="'project_' + index" :value="project.id">{{
                project.title
                }}</option>
            </select>
            <svg
        ...
        <div class="mb-4" v-if="donation_types && donation_types.donation_types">
          <label for="select-donation-type"
            class="required text-green dark:text-black font-bold block mb-2 text-lg">Donation Type</label>
          <div class="grid">
            <select @change="addDonationType" id="select-donation-type" v-model="current_donation.donation_type_id"
              class="appearance-none row-start-1 col-start-1 form-select block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-green focus:outline-none"
              aria-label="Donation Types" :class="{ '!border-red': errors && errors.donation_type_id }">
              <option value="">Select Donation Type</option>
              <option v-for="(type, index) in donation_types.donation_types" :key="'type_' + index" :value="type.id">{{
                type.name }}</option>
            </select>
            <svg
              class="pointer-events-none z-10 right-1 relative col-start-1 row-start-1 h-4 w-4 self-center justify-self-end forced-colors:hidden"
        ...
        <div class="mb-4">
          <label for="" class=" text-green dark:text-black font-bold block mb-2 text-lg"
            v-if="current_donation && current_donation.project">Frequency</label>
          <ul class="flex gap-2" v-if="current_donation && current_donation.project">
            <template v-for="(frequency, index) in frequencies" :key="index">
              <li class="relative">
                <input class="sr-only peer" :disabled="!checkIfAllowed(frequency.name)" type="radio"
                  v-model="current_donation.monthly" name="frequency" :value="frequency.selected"
                  :id="'frequency_' + index">
                <label :class="{
        ...
        <div class="mb-4" v-if="current_donation.project && amounts && (amounts.length > 0)">
          <label for="select-amounts" class=" text-green dark:text-black font-bold block mb-2 text-lg">Donation
            amount</label>
          <ul class="flex gap-2">
            <li class="relative" v-for="(amount, index) in amounts" :key="index">
              <input class="sr-only peer" type="radio" v-model="current_donation.fix_amount" :value="parseFloat(amount)"
                name="amount" :id="'amount_' + amount">
              <label
                class="select-none flex px-4 py-2 bg-white border border-gray-300 rounded-lg cursor-pointer focus:outline-none hover:bg-gray-50 peer-checked:ring-green dark:peer-checked:ring-black peer-checked:ring-2 peer-checked:border-transparent"
                :for="'amount_' + amount">{{ $formatAmount(amount) }}</label>
        ...
        <div class="mb-4 flex justify-between">
          <button @click="viewBasket()" type="button"
            class="p-2 md:px-6 pt-2.5 pb-2 bg-gray-600 text-white font-medium text-xs leading-normal uppercase rounded shadow-md hover:bg-green dark:hover:bg-black hover:shadow-lg focus:bg-green dark:focus:bg-black focus:shadow-lg focus:outline-none focus:ring-0 dark:active:bg-black active:bg-green active:shadow-lg transition duration-300 ease-in-out flex align-center items-center">
          ...
          <button @click="addDonation" type="button"
            class="p-2 md:px-6 pt-2.5 pb-2 bg-red dark:bg-black text-white font-medium text-xs leading-normal uppercase rounded shadow-md hover:bg-green dark:hover:bg-gray-700 hover:shadow-lg focus:bg-green dark:focus:bg-gray-800 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-green dark:active:bg-black active:shadow-lg transition duration-300 ease-in-out flex align-center items-center">
            Add Donation
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="w-5 ml-3">
              <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="mb-4">
          <div v-if="show_alert"
            class="bg-green-100 border-2 border-teal-700 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
...
export default {
  ...
  methods: {
    projectChanged() {
      ...
      // get fix amounts based on selected currency
      if (this.form.selected_currency == "GBP") {
        this.amounts = (this.current_donation.project && this.current_donation.project.fix_amounts) ? this.current_donation.project.fix_amounts.split(',') : []
      } else if (this.form.selected_currency == "EUR") {
        this.amounts = (this.current_donation.project && this.current_donation.project.fix_amounts_eur) ? this.current_donation.project.fix_amounts_eur.split(',') : []
      } else if (this.form.selected_currency == "USD") {
        this.amounts = (this.current_donation.project && this.current_donation.project.fix_amounts_usd) ? this.current_donation.project.fix_amounts_usd.split(',') : []
      }

      // Set GBP As Default if Amounts not present
      if (this.amounts.length == 0) {
        this.amounts = (this.current_donation.project && this.current_donation.project.fix_amounts) ? this.current_donation.project.fix_amounts.split(',') : []
      }

      if (this.amounts.length > 0) {
        this.current_donation.fix_amount = parseFloat(this.amounts[0])
      }
