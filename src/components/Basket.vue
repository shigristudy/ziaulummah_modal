  ...
  <div class="pointer-events-auto">
    <div class="flex h-full flex-col bg-gray-100">
      <div class="basket" v-if="step == 1">
                      ...
                      <div>
                        <div class="flex justify-between text-base font-bold text-gray-900">
                          ...
                          <template class="ml-4 text-lg flex justify-center items-center gap-2 mb-0">
                            ...
                            {{ donation.qty }}
                            =
                            {{ $formatAmount(donation.amount ? donation.amount * donation.qty : donation.fix_amount *
                              donation.qty) }}
                          </template>
                        </div>
                      ...
                      <div class="flex flex-1 items-end justify-between text-sm">
                        ...
                        <div class="flex justify-center items-center gap-2">
                          <div class="border border-gray-300">
                            <button class="px-2 bg-gray-200 border-r border-gray-300" :disabled="donation.qty <= 1"
                              @click="donation.qty--">
                              -
                            </button>
        ...
        <div class="mb-4 px-4 sm:px-6" v-if="!all_monthly">
          <ul class="flex gap-2">
            <li
              ...
              <input class="sr-only peer" type="checkbox" v-model="isSelectedPaperCopy" id="admin_fee_cover"
                @change="addCustomProject('paper_copy')" />
              <label for="admin_fee_cover" :class="{
                ...
                <svg v-if="isSelectedPaperCopy" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            ...
            <li
              ...
              <input class="sr-only peer" type="checkbox" v-model="isAdminFeeSelected" id="paper_copy"
                @change="addCustomProject('admin_fee')" />
              <label for="paper_copy" :class="{
                ...
                <svg v-if="isAdminFeeSelected" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                  stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-2">
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
        ...
        <div class="py-6 px-4 sm:px-6">
          ...
          <div class="mt-6 flex justify-between">
            <button type="button" @click="makeAnother()"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-4 md:w-5 mr-1 md:mr-2">
            ...
            <button :disabled="!donations.length" type="button" @click="moveForward()" :class="{
              ...
              Proceed to Checkout
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-5 ml-1 md:ml-2">
                <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <div class="checkout px-6" v-if="step == 2">
        ...
        <div class="relative">
          <input placeholder="Address Line 1" v-model="form.address_line_1"
        ...
        <input placeholder="Address Line 2" v-model="form.address_line_2"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
          <div class="relative">
            <input placeholder="City" v-model="form.city"
          ...
          <div class="relative">
            <input placeholder="Postal Code/Zip Code" v-model="form.post_code"
        ...
        <input @keypress="onlyNumber" placeholder="Phone Number" v-model="form.phone"
        ...
        <textarea
          class="mb-2 form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-green focus:outline-none"
          placeholder="Notes (On behalf of)" v-model="form.notes" rows="3"></textarea>
        ...
        <div class="border-green border p-3 mt-4">
          ...
          <div class="mb-2 flex items-center gap-1">
            <input id="agreed" type="checkbox" v-model="form.gift_aid" :value="1" />
            <label for="agreed" class="ml-1">I agree, I would like to boost my donation with Gift Aid
            </label>
          </div>
        </div>
        ...
        <div class="my-2">
          ...
          <div class="grid grid-cols-2">
            <div class="form-check">
              <input v-model="form.contact_medium" value="Post" type="checkbox" name="contact_medium"
                id="contact_medium_1" />
              <label class="ml-2" for="contact_medium_1"> Post </label>
            </div>
            <div class="form-check">
              <input v-model="form.contact_medium" value="Email" type="checkbox" name="contact_medium"
                id="contact_medium_2" />
              <label class="ml-2" for="contact_medium_2"> Email </label>
            </div>
            <div class="form-check">
              <input v-model="form.contact_medium" value="Phone" type="checkbox" name="contact_medium"
                id="contact_medium_3" />
              <label class="ml-2" for="contact_medium_3"> Phone </label>
            </div>
            <div class="form-check">
              <input v-model="form.contact_medium" value="SMS" type="checkbox" name="contact_medium"
                id="contact_medium_4" />
        ...
        <hr />
        <div class="my-2 flex items-center border border-green p-3"
          ...
          <label for="synergidigital-accept-terms-business" class="ml-1 required text-sm">
            By checking this box, you are agreeing to our
            <a target="__blank" :href="policyLink" class="text-red">terms of service</a>
          </label>
        </div>
        <hr />
        <div class="mt-4">
          ...
          <div class="flex gap-6 flex-col">
            <div class="flex items-center gap-2" v-if="
              ...
              containsOneOff
            ">
              <input id="radio1" type="radio" v-model="form.payment_type" name="method" value="Credit Card" />
            ...
            <div class="flex items-center gap-2" v-if="isGatewayEnabled('paypal') && !hasMonthly">
              <input :disabled="hasMonthly" id="radio2" type="radio" v-model="form.payment_type" value="PayPal"
                name="method" />
              <label for="radio2" class="flex gap-1 items-center">
                <IconPaypalCard />
                <span v-if="hasMonthly" class="text-danger text-sm">Paypal is not available if any subscription product
                  exist
                  in
                  your basket</span>
            ...
            <div class="flex items-center gap-2" v-if="isGatewayEnabled('gocardless') && !containsOneOff">
              <input :disabled="containsOneOff" id="radio3" type="radio" v-model="form.payment_type" name="method"
                value="Gocardless" />
        ...
        <div class="my-6 flex justify-between">
          <button type="button" @click="moveBack()"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="w-5 mr-1 md:mr-3">
              <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <div class="checkout px-6" v-if="step == 3">
        ...
        <Paypal @moveBack="moveBack()" :donations="donations" :paypalPublicKey="getGatewayPublicKey('paypal')"
